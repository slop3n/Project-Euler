...
class Program
{
    ...
    static void Main(string[] args)
    {
        ...
        for (int row = 0; row < matrix.GetLength(0) - 3; row++)
        {
            ...
            {
                //rows
                if (matrix[row, col] * matrix[row + 1, col] * matrix[row + 2, col] * matrix[row + 3, col] > product)
                {
                    product = matrix[row, col] * matrix[row + 1, col] * matrix[row + 2, col] * matrix[row + 3, col];
                }
                //cols
                if (matrix[row, col] * matrix[row, col + 1] * matrix[row, col + 2] * matrix[row, col + 3] > product)
                {
                    product = matrix[row, col] * matrix[row, col + 1] * matrix[row, col + 2] * matrix[row, col + 3];
                }
                //diagonal
                if (matrix[row, col] * matrix[row + 1, col + 1] * matrix[row + 2, col + 2] * matrix[row + 3, col + 3] > product)
                {
